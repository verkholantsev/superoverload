...
function overload() {
    ...
    var code = [
        ...
        .concat([
            pairs.length && ' else {if (!defaultFn) {throw new Error(\'No matching function for call with signature "\' + hashKey + \'"\');}} return defaultFn.apply(this, arguments);'
        ])

    var superCode = [
            'return function(' + maxArgs.map(function(arg, argIndex) { return '_' + argIndex}).join(',') + ') {'
        ]
        .concat(code)
        .concat([
            '}'
