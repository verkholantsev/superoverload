!(function(n, r) {
  ...
  function r(r) {
    ...
    var e = Object.prototype.toString.call(r).match(n);
    if (!Array.isArray(e))
      throw new Error('Unexpected type of arg '.concat(String(r)));
  ...
  return function() {
    for (var n = arguments.length, o = new Array(n), i = 0; i < n; i++)
      o[i] = arguments[i];
    for (
      var u = o.length % 2 > 0 ? o.shift() : null,
        c = e(o),
      ...
      if (p.length > 0)
        throw new Error(
          'Signature "'
            .concat(d.join(', '), '" contains unsupported types: "')
            .concat(p.join(', '), '"'),
        );
      var v = d.join(', ');
      (l[s] = y),
        d.length > h.length && (h = d),
        (f[s] = "if (hashKey === '"
          .concat(v, "') {return fns[")
          .concat(String(s), '].call(this, ')
          .concat(t(d), ');}'));
    }
    var w = f.join(' else '),
      m = t(h),
      j = 'return function overloadedFn('
        .concat(
          m,
      ...
      A = new Function('getType, fns, defaultFn', j);
    return A(r, l, u);
  };
});
